/**
 * Tracking da LP /condominio (Smart Locker para condominio).
 *
 * Este modulo NAO carrega nenhum script. GTM, GA4, Meta Pixel, HubSpot e LinkedIn
 * ja vivem no app/layout.tsx. Aqui so empurramos evento para os globais que ja existem,
 * sempre com guarda de `typeof window` e try/catch: se o global nao estiver la
 * (bloqueador, SSR, consentimento negado), a pagina segue funcionando.
 *
 * Portado de lp-stoom-oficial/index.html, secao <script> final.
 */
 ...
 * CONFIRMAR com a Stoom antes de publicar.
 */
  ...
  if (typeof w.fbq === 'function') {
    try {
      w.fbq(
        'track',
        'Lead',
        { content_name: 'Smart Locker Condominio', content_category: 'condominio' },
        { eventID: eventId }
      )
    } catch {
